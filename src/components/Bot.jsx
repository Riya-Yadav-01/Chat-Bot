...
function Bot() {
  ...
  const handleSendMessage = async () => {
    ...
    try{
      const res= axios.post("http://localhost:4002/bot/v1/message", {
        text: input
      })
      if(res.status === 200) {
        setMessages(prevMessages => [...prevMessages, { text: res.data.userMessage, sender: 'user'}, {text: res.data.botMessage, sender: 'bot'}]);
      }
